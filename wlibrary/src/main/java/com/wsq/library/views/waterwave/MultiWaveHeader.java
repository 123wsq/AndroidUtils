package com.wsq.library.views.waterwave;
...
public class MultiWaveHeader extends ViewGroup {
    ...
    public MultiWaveHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mWaveHeight = ta.getDimensionPixelOffset(R.styleable.MultiWaveHeader_mwhWaveHeight, Wave.dp2px(50));
...
//    }
    ...
    private void updateWavePath(int w, int h) {
        ...
        if (count > 0) {
            ...
            for (String wave : waves) {
                ...
                if (args.length == 5) {
                    mltWave.add(new Wave(/*getContext(),*/
                            Wave.dp2px(parseFloat(args[0])), Wave.dp2px(parseFloat(args[1])),
                            Wave.dp2px(parseFloat(args[4])), parseFloat(args[2]),
                            parseFloat(args[3]), w, h, mWaveHeight/2));
                }
            }
        } else {
            mltWave.add(new Wave(/*getContext(),*/
    ...
    public void setWaveHeight(int waveHeight) {
        this.mWaveHeight = Wave.dp2px(waveHeight);
        if (!mltWave.isEmpty()) {
            View thisView = this;
            updateWavePath(thisView.getWidth(), thisView.getHeight());
        }
